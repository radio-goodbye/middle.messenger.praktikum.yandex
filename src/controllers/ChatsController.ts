...
export class ChatsController extends Controller{
    ...
    createChat(name: string){
        this.chatList.create({name}).then((data) => {
            this.getChats();
        })
    }
    ...
    selectChat(id: number){
        ...
        this.chatList.request().then(data => {
            ...
            if (chat){
                let chat_state = {id, title: chat.title, avatar: chat.avatar} as ChatStateModel;

                this.chatTokenApi.request({id})
                .then(data => {
                    chat_state.token = data.token;
                    return this.chatUsersApi.request({id});
                })
                .then(data => {
                    chat_state.users= data;
                    this.store.set('current_chat', chat_state, 'chats:selected');
                });
            }
            else{
